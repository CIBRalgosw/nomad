...
def get_data(filepath, filename, bin_size=0.02, smooth_size=0, bhv_type='vel'):
    """
    ...
    spike = data.get_trials_data_spike_counts('R')
    ...
    if bhv_type == 'force':
        force = data.get_trials_data_force('R')
        bhv = slide_window(force)
    if bhv_type == 'vel':
        _, cursor_velocity, _ = data.get_trials_data_cursor('R')
        bhv = slide_window(cursor_velocity)
...
if __name__ == "__main__":
    ...
    spike, bhv = get_data(data_path, file_name, bhv_type=bhv_type)
